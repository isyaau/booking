...
$con = connect();
	        	...
	        	if (isset($_FILES['image'])) {
				 // files handle
				    ...
				    if ($extension =="jpg" || $extension =="png" || $extension =="jpeg"){
				    	...
				    	$iquery="INSERT INTO `user`(`nama`, `email`, `phone`, `address`, `logo`, `password`, `role`) 
        	...
        	echo '<script>window.location="register.php"</script>';
	        	...
	        	if (isset($_FILES['image'])) {
				 // files handle
				    ...
				    if ($extension =="jpg" || $extension =="png" || $extension =="jpeg"){
				    	move_uploaded_file($file_tmp,$targetDirectory.$file_name);
				    	$iquery="INSERT INTO `user`(`nama`, `email`, `phone`, `address`, `location`, `logo`, `password`, `role`) 
    ...
    if (isset($_POST['book'])) {
		...
		$iquery="INSERT INTO `reservasi`(`booking_id`,`res_id`,`c_id`,`make_date`, `make_time`, `name`, `phone`, `booking_date`, `booking_time`, `bill`,`layout`,`catatan`,`transactionid`) 
		    VALUES ('$booking_id','$res_id','$u_id','$make_date','$make_time','$reservation_name','$reservation_phone','$reservation_date','$reservation_time','$total_price', '$layout', '$catatan','$transactionid');";
		if ($con->query($iquery) === TRUE) {
			$bdinsert = true;
		}else {
            echo "Error: " . $iquery . "<br>" . $con->error;
        ...
        if ($bdinsert == true) {
        	for($q = 0; $q < count($_POST["chair"]); $q++){
                ...
                $ciQuery = "INSERT INTO `kursi_booking`(`booking_id`, `chair_id`, `chair_no`) 
